...
;
; :REQUIRED:
;    BB_RADIANCE (included)
;    READ_DP_FILE (included)
;
; :MODIFICATION HISTORY:
;    Written by:       Carl Salvaggio
;    June, 2012        Original code
;    October, 2025     Modifications to handle various wavenumber 
;                      resolutions and zero padding values
;
; :DISCLAIMER:
...
PRO DP_RADIANCE_CALIBRATION_COMPUTE, state
   ...
   spectrumSize = 512 * sampleHeader.FFTSize * sampleHeader.ZeroFill
   ...
   individualWavelength = FLTARR(spectrumSize)
   individualSlope = FLTARR(spectrumSize)
   individualOffset = FLTARR(spectrumSize)
   FOR iWavelength = 0, spectrumSize - 1 DO BEGIN
      individualWavelength[iWavelength] = sampleSpectrum.Wavelength[iWavelength]
      individualSlope[iWavelength] = calibrationSlope[iWavelength]
      individualOffset[iWavelength] = calibrationOffset[iWavelength]
   ENDFOR


   individualCBB = REPLICATE( { spectra:FLTARR(spectrumSize) }, cbbHeader.NumberOfCoAdds )
   ...
   FOR scan = 0, cbbHeader.NumberOfCoadds-1 DO BEGIN
      ...
      centeredInterferogram = SHIFT( cbbInterferogram[scan].Value, $
                                     interferogramShift )
      centeredInterferogram = $
         centeredInterferogram[cbbHeader.InterferogramSize/2-2048: $
   ...
   individualWBB = REPLICATE( { spectra:FLTARR(spectrumSize) }, wbbHeader.NumberOfCoAdds )
   ...
   individualSample = REPLICATE( { spectra:FLTARR(spectrumSize) }, sampleHeader.NumberOfCoAdds )
   ...
   IF ( downwellingProvided ) THEN BEGIN
      individualDownwelling = REPLICATE( { spectra:FLTARR(spectrumSize) }, downwellingHeader.NumberOfCoAdds )
   ...
   IF ( createASCII ) THEN BEGIN
      ...
      PRINTF, sLun, $
              ...
              dataSource, $
              ']'
      FOR I = 0, spectrumSize-1, sampleHeader.ZEROFILL DO BEGIN
      ...
      IF ( downwellingProvided ) THEN BEGIN
         ...
         PRINTF, dLun, $
                 ...
                 dataSource, $
                 ']'
         FOR I = 0, spectrumSize-1, downwellingHeader.ZEROFILL DO BEGIN
...
PRO READ_DP_FILE, filename, device, HEADER=headerData, $
   ...
   spectrumSize = 512 * headerData.FFTSize * headerData.ZeroFill

   interferogram = REPLICATE( { FREQUENCY:FLTARR(headerData.InterferogramSize), $
                                VALUE:INTARR(headerData.InterferogramSize) }, headerData.NumberOfCoAdds )
   iValue = INTARR(headerData.InterferogramSize)

   FOR I = 0, headerData.NumberOfCoAdds-1 DO BEGIN
      READU, LUN, iValue
      FOR J = 0, headerData.InterferogramSize-1 DO BEGIN
         iValue[J] = SWAP_ENDIAN( iValue[J], /SWAP_IF_BIG_ENDIAN )
      ENDFOR
      interferogram[I].VALUE = iValue
   ENDFOR

   spectrum = { WAVELENGTH:FLTARR(spectrumSize), VALUE:FLTARR(spectrumSize) }
   ...
   FOR I = 0, (headerData.InterferogramSize/2-1) DO BEGIN
      interferogram.FREQUENCY[I] = largestWavenumber - ((headerData.InterferogramSize/2)-I) * deltaWavenumber
